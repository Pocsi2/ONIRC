...
function previewDream(body: string, date: string, title: string): Dream {
  return {
    ...visualSavedDream,
    date: date || visualSavedDream.date,
    title: title || "Untitled dream",
    body: body || visualSavedDream.body,
...
export function NewDreamForm() {
  ...
  const [body, setBody] = React.useState(
    "I was walking through a white garden where every flower opened into a small room.",
  );
  const [date, setDate] = React.useState("2026-08-28");
  const [title, setTitle] = React.useState("The White Garden");
  const [kept, setKept] = React.useState(false);

  const canKeep = body.trim().length > 8 && Boolean(date) && title.trim().length > 1;
  const dream = previewDream(body, date, title);
    ...
    <PageTransition className="mx-auto grid max-w-6xl gap-10 lg:grid-cols-[minmax(0,1fr)_360px]">
      ...
      >
        ...
        <div className="relative">
          ...
          <h1 className="mt-5 max-w-3xl font-display text-[clamp(3.4rem,8vw,7.4rem)] leading-[.88] tracking-[-0.055em]">
            Let&apos;s keep one.
          </h1>
          ...
          >
            <Reveal delay={0.1}>
              <label className="block">
                ...
                <Textarea
                  ...
                  placeholder="Write the fragment before it fades."
                ...
                >
                  ...
                  <label className="block">
                    ...
                    <Input
                      ...
                      placeholder="A title for the memory"
            ...
            <div className="flex flex-col gap-3 border-t border-white/55 pt-8 sm:flex-row sm:items-center">
              ...
              <Button asChild variant="ghost" type="button">
                <ViewTransitionLink href="/calendar">Return to calendar</ViewTransitionLink>
              </Button>
            </div>
          </form>
        </div>
      ...
      >
        <div className="surface-frost rounded-[42px] p-7">
          <div className="mb-8 flex items-center justify-between">
            <p className="text-xs uppercase tracking-[0.26em] text-text-muted">Preview</p>
          ...
          <p className="mt-5 line-clamp-5 text-sm leading-7 text-text-secondary">
            {body || "The memory will appear here as you write."}
          </p>
            ...
            {kept ? (
              ...
              >
                <div className="flex items-center gap-3 text-sm text-text-secondary">
                  <span className="grid h-8 w-8 place-items-center rounded-full bg-mist-mint/70">
                    <Check className="h-4 w-4 text-text-primary" />
                  </span>
                  Kept. It now lives in your temporal landscape.
                </div>
                <Button asChild variant="secondary" className="w-full">
                  <ViewTransitionLink href="/calendar?kept=white-garden">
                    See where it lives
                  </ViewTransitionLink>
                </Button>
              </motion.div>
              ...
              >
                Write the fragment first. The pearl will form as you remember.
              </motion.p>
            )}
          </AnimatePresence>
        </div>
