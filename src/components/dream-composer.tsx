      ...
      >
        <div className="mb-8 flex items-start justify-between gap-5">
          <div>
            ...
            <h2 id="dream-composer-title" className="mt-3 font-display text-5xl leading-[.92] tracking-[-0.045em]">{isEdit ? "Volvamos a este momento." : "Escribe antes de que se vaya."}</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-6" noValidate>
          <label htmlFor="dream-body" className="block">
            ...
            <Textarea id="dream-body" ref={bodyRef} autoFocus value={body} onChange={(event) => updateField(setBody, event.target.value)} placeholder="Escribe el fragmento antes de que se disuelva." className="min-h-48" aria-invalid={Boolean(error && body.trim().length < 8)} />
          </label>
            ...
            {revealDetails ? (
              <motion.div initial={reducedMotion ? { opacity: 0 } : { opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -4 }} transition={reducedMotion ? reducedTransition : transitions.standard} className="grid gap-5 sm:grid-cols-[.78fr_1.22fr]">
                ...
                <label htmlFor="dream-title" className="block">
                  <span className="mb-2 block text-sm font-medium text-text-secondary">Dale un nombre</span>
                  <Input id="dream-title" ref={titleRef} value={title} onChange={(event) => updateField(setTitle, event.target.value)} placeholder="Una frase para volver" maxLength={120} aria-invalid={Boolean(error && title.trim().length < 2)} />
                </label>
              </motion.div>
            ) : null}
          </AnimatePresence>
          ...
          <div className="flex flex-col-reverse gap-3 border-t border-[var(--border-quiet)] pt-6 sm:flex-row sm:items-center sm:justify-between">
            ...
            <Button type="submit"><Feather className="h-4 w-4" />{isEdit ? "Conservar cambios" : "Guardar en el tiempo"}</Button>
          </div>
        </form>
